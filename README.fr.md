...
<img src="src/icons/app-icon.png" alt="Icône Cine Power Planner" width="200">
...
- **Récapitulatifs de compatibilité de restauration.** Chaque restauration exécute un contrôle de compatibilité qui liste les sections manquantes et le nom de la sauvegarde de sécurité préalable, afin que les équipes confirment les changements avant de promouvoir les données.
- **Fonctionnement hors ligne garanti.** Toutes les icônes, polices, Uicons et scripts fournis voyagent avec le dépôt. Autosave, backup, restauration, partage et aide fonctionnent intégralement en local pour travailler sereinement en environnement isolé.
- **Coffre-fort local de sauvegardes en cas de téléchargements bloqués.** Lorsque les téléchargements automatiques sont bloqués, le planner met les sauvegardes en file dans un coffre-fort local et affiche une action dans la bannière pour les exporter sans perdre de capture.
...
| Récapitulatif de compatibilité de restauration | Avertit lorsqu’une sauvegarde manque de sections modernes avant promotion. | Restaurer via le bac à sable et relire l’alerte de compatibilité avant de continuer. | Capture de la liste des sections manquantes et du nom de la sauvegarde de sécurité. |
| File d’attente du coffre-fort local de sauvegardes | Capture les auto-backups lorsque le navigateur bloque les téléchargements. | Utiliser l’action de la bannière pour ouvrir le coffre-fort local et exporter les fichiers JSON en attente. | Journaux d’export du coffre-fort et sauvegardes en attente archivées. |
