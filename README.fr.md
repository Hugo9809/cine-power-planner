...
- **Recherche globale** (`/`, `Ctrl+K`, `⌘K`) pour accéder à toute fonctionnalité, liste ou thème d’aide, même navigation repliée.
  ...
  d’aide pour que les parcours clavier atteignent d’abord les commandes essentielles. Les mots correspondants sont surlignés dans
  la liste déroulante afin d’expliquer immédiatement chaque résultat. Lorsque le champ est vidé, les correspondances récemment
  utilisées remontent en tête afin de répéter les routines d’équipe en un clin d’œil.
