...
const schema = require('../web/data/schema.json');
...
function findMissingAttributes(categoryPath, dataset = data, attributeMap = ATTRIBUTE_MAP) {
  ...
  if (!attributes) {
    throw new Error(`Category "${categoryPath}" is not defined in web/data/schema.json`);
  }
  ...
  if (!categoryArgs.length && !checkAll) {
    console.log(
      [
        ...
        '',
        'Options:',
        '  --all        Check every category defined in web/data/schema.json',
        '  --json       Output results as JSON',
        '  --list       List all category paths available in web/data/schema.json',
        '',
        'Examples:',
