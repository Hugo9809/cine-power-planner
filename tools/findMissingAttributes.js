...
function findMissingAttributes(categoryPath, dataset = data, attributeMap = ATTRIBUTE_MAP) {
  ...
  if (!attributes) {
    throw new Error(`Category "${categoryPath}" is not defined in public/data/schema.json`);
  }
  ...
  if (!categoryArgs.length && !checkAll) {
    console.log(
      [
        ...
        '',
        'Options:',
        '  --all        Check every category defined in public/data/schema.json',
        '  --json       Output results as JSON',
        '  --list       List all category paths available in public/data/schema.json',
        '',
        'Examples:',
