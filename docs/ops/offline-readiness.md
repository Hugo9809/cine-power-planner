...
- [ ] Use the queued-backup banner action (or open **Settings → Backup & Restore
      → Open local backup vault**) to confirm no queued backups remain. Log any
...
2. **Midday** – Rotate backups to second storage device; record actions in
   `../dev/reviews/review-findings.md`.
