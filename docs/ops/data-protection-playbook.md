...
| Autosave cadence | `src/scripts/app-session.js` schedules and dispatches background autosaves every ~50 changes or 10 minutes, while `modules/persistence.js` handles downstream storage and ledger recording. The logger is resolved through `modules/logging-resolver.js` so diagnostics are captured even when the structured logging module is unavailable. | Ledger export plus timestamped console log. |
...
5. Export the verification packet, planner backup and project bundles. Store two
   physical copies and log their locations in `../dev/reviews/review-findings.md`.
...
3. **Document impact.** Update `../dev/reviews/review-findings.md` and `feature-gap-analysis.md`
...
- Track outstanding risks in `../dev/reviews/review-findings.md` and link to mitigation plans in
