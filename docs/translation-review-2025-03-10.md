...
Document each localisation approval and rerun `npm run test:unit -- tests/unit/autoGearCameraWeight.test.js` before shipping to keep offline bundles in sync. This single-suite run exercises the camera-weight translation guard that fails whenever a locale misses new weight descriptors, ensuring locale fixes continue to protect offline bundles.
