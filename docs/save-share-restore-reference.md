...
4. `window.getMountVoltagePreferencesClone()` / `window.applyMountVoltagePreferences()` – confirm these helpers remain globally bound so autosave, share and restore flows can snapshot and reinstate mount voltage thresholds without duplicating logic. They should return deep clones, accept temporary updates without persisting during rehearsals, and reset cleanly via `window.resetMountVoltagePreferences({ persist: false })`.【F:src/scripts/app-core-new-1.js†L1316-L1419】【F:tests/dom/sharedProjectGearList.test.js†L14-L71】
