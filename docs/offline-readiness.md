...
- **Logger retention counters.** Capture the snapshot from `cineLogging.getStats()` so the
  ...
  history, which severities dominated and how many events were preserved. The expanded
  default retention now keeps roughly 1,200 entries before rotation and the per-level
  breakdown shows whether a purge mostly removed debug chatter or critical warnings.
  Keeping this metadata with the exported backups proves that no warnings disappeared
  silently while the limit rotated entries.
