...
function collectBootstrapFallbackScopes(extraScopes) {
  const fallbackScopes = Array.isArray(extraScopes) ? extraScopes.slice() : [];
  ...
  enqueue(getDefaultRuntimeScope());
  enqueue(getDefaultCoreGlobalScope());
...
function resolveBootstrapResolverTools() {
  if (hasBootstrapResolverCapabilities(APP_CORE_BOOTSTRAP_RESOLVER_DIRECT)) {
    return APP_CORE_BOOTSTRAP_RESOLVER_DIRECT;
  }
  ...
  if (typeof requireFn === 'function') {
    try {
      ...
      if (
        requiredResolver &&
        typeof requiredResolver === 'object' &&
...
const APP_CORE_BOOTSTRAP_RESOLVER_TOOLS = resolveBootstrapResolverTools();

function resolveAppCoreBootstrapTools() {
  const resolverTools = APP_CORE_BOOTSTRAP_RESOLVER_TOOLS;

  if (resolverTools && typeof resolverTools.resolveBootstrapTools === 'function') {
    try {
      const resolved = resolverTools.resolveBootstrapTools({
        directNamespace: APP_CORE_BOOTSTRAP_TOOLS,
        ...
        fallbackScopes: collectBootstrapFallbackScopes(),
      });
  ...
  if (APP_CORE_BOOTSTRAP_TOOLS && typeof APP_CORE_BOOTSTRAP_TOOLS === 'object') {
    return APP_CORE_BOOTSTRAP_TOOLS;
  }
...
function resolveAppCoreBootstrapFallbackTools() {
  ...
  if (resolverTools && typeof resolverTools.resolveBootstrapFallbackTools === 'function') {
    try {
      const resolved = resolverTools.resolveBootstrapFallbackTools({
        directNamespace: resolveCoreSupportModule(
...
function createInlineLocalizationFallback() {
  ...
  if (resolverTools && typeof resolverTools.createInlineLocalizationFallback === 'function') {
    try {
      const generated = resolverTools.createInlineLocalizationFallback({
...
function createInlineRuntimeSharedFallback(fallbackOptions) {
  ...
  const extraFallbackScopes = [];

  if (runtimeScope) {
    extraFallbackScopes.push(runtimeScope);
  }

  if (coreGlobalScope && extraFallbackScopes.indexOf(coreGlobalScope) === -1) {
    extraFallbackScopes.push(coreGlobalScope);
  }

  if (fallbackOptions && Array.isArray(fallbackOptions.fallbackScopes)) {
    for (let index = 0; index < fallbackOptions.fallbackScopes.length; index += 1) {
      const scope = fallbackOptions.fallbackScopes[index];

      if (!scope || typeof scope !== 'object') {
        continue;
      }

      if (extraFallbackScopes.indexOf(scope) === -1) {
        extraFallbackScopes.push(scope);
      }
    }
  }

  const fallbackScopes = collectBootstrapFallbackScopes(extraFallbackScopes);

  const resolverTools = APP_CORE_BOOTSTRAP_RESOLVER_TOOLS;

  if (resolverTools && typeof resolverTools.createInlineRuntimeSharedFallback === 'function') {
    try {
      const generated = resolverTools.createInlineRuntimeSharedFallback({
        fallbackTools: APP_CORE_BOOTSTRAP_FALLBACK_TOOLS,
        resolveCoreSupportModule,
        requireFn: typeof require === 'function' ? require : null,
        runtimeScope,
  ...
  if (
    ...
    try {
      const moduleFallback =
        APP_CORE_BOOTSTRAP_FALLBACK_TOOLS.createRuntimeSharedBootstrapFallback(fallbackOptions);
  ...
  let runtimeShared =
    fallbackOptions &&
    fallbackOptions.currentRuntimeShared &&
    typeof fallbackOptions.currentRuntimeShared === 'object'
      ? fallbackOptions.currentRuntimeShared
      : null;
...
const localizationBootstrapResult = (function resolveLocalizationBootstrapResult() {
  ...
  const bootstrapTools = resolveAppCoreBootstrapTools();

  if (bootstrapTools && typeof bootstrapTools.createLocalizationBootstrapResult === 'function') {
    try {
      const resolved = bootstrapTools.createLocalizationBootstrapResult(bootstrapOptions);
  ...
  if (bootstrapTools && typeof bootstrapTools.createLocalizationBootstrapFallback === 'function') {
    try {
      const fallbackResult = bootstrapTools.createLocalizationBootstrapFallback(bootstrapOptions);
...
const APP_CORE_LOCALIZATION_SUPPORT =
  localizationBootstrapResult && localizationBootstrapResult.localizationSupport
    ? localizationBootstrapResult.localizationSupport
    : null;

const localizationRuntimeEnvironment =
  localizationBootstrapResult && localizationBootstrapResult.localizationRuntimeEnvironment
    ? localizationBootstrapResult.localizationRuntimeEnvironment
    : null;

const CORE_LOCALIZATION_BRIDGE =
  localizationBootstrapResult && localizationBootstrapResult.localizationBridge
    ? localizationBootstrapResult.localizationBridge
    : null;

const CORE_LOCALIZATION_FALLBACKS =
  localizationBootstrapResult && localizationBootstrapResult.localizationFallbacks
    ? localizationBootstrapResult.localizationFallbacks
    : null;

const CORE_INLINE_LOCALIZATION_FALLBACKS =
  localizationBootstrapResult && localizationBootstrapResult.inlineLocalizationFallbacks
    ? localizationBootstrapResult.inlineLocalizationFallbacks
    : null;

const LOCALIZATION_FALLBACK_SUPPORT =
  localizationBootstrapResult &&
  typeof localizationBootstrapResult.localizationFallbackSupport !== 'undefined'
    ? localizationBootstrapResult.localizationFallbackSupport
    : null;

const createBasicLocalizationFallbackResolvers =
  localizationBootstrapResult &&
  typeof localizationBootstrapResult.createBasicLocalizationFallbackResolvers === 'function'
    ? localizationBootstrapResult.createBasicLocalizationFallbackResolvers
    : function createBasicLocalizationFallbackResolversProxy() {
        return null;
      };

const LOCALIZATION_FALLBACK_REGISTRY =
  localizationBootstrapResult && localizationBootstrapResult.localizationFallbackRegistry
    ? localizationBootstrapResult.localizationFallbackRegistry
    : {
...
const LOCALIZATION_FALLBACK_RESOLVERS =
  localizationBootstrapResult && localizationBootstrapResult.localizationFallbackResolvers
    ? localizationBootstrapResult.localizationFallbackResolvers
...
const LOCALIZATION_FALLBACK_NAMESPACE =
  localizationBootstrapResult &&
  typeof localizationBootstrapResult.localizationFallbackNamespace !== 'undefined'
    ? localizationBootstrapResult.localizationFallbackNamespace
    : null;

const fallbackResolveLocaleModule =
  localizationBootstrapResult &&
  typeof localizationBootstrapResult.fallbackResolveLocaleModule === 'function'
    ? localizationBootstrapResult.fallbackResolveLocaleModule
...
const createLocaleFallbacks =
  localizationBootstrapResult &&
  typeof localizationBootstrapResult.createLocaleFallbacks === 'function'
    ? localizationBootstrapResult.createLocaleFallbacks
...
const runtimeSharedBootstrapResult = (function resolveRuntimeSharedBootstrapResult() {
  ...
  if (bootstrapTools && typeof bootstrapTools.createRuntimeSharedBootstrapResult === 'function') {
    try {
      const resolved = bootstrapTools.createRuntimeSharedBootstrapResult(bootstrapOptions);
  ...
  if (bootstrapTools && typeof bootstrapTools.createRuntimeSharedBootstrapFallback === 'function') {
    try {
      const fallbackResult = bootstrapTools.createRuntimeSharedBootstrapFallback(bootstrapOptions);
  ...
  return createInlineRuntimeSharedFallback(bootstrapOptions);
})();
