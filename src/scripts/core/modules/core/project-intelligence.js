...
function installProjectIntelligence(payload) {
  ...
  for (let index = 0; index < PROJECT_EXPORTS.length; index += 1) {
    const name = PROJECT_EXPORTS[index];
...
const frozenProjectAPI = Object.freeze(projectAPI);
...
const exposed = exposeGlobal('cineCoreProject', frozenProjectAPI, {
  configurable: true,
  enumerable: false,
  writable: false,
});
...
export const cineCoreProject = frozenProjectAPI;
