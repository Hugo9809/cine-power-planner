/* global activateSettingsTab, openDialog, closeDialog, isDialogOpen */

import { cineModuleBase as MODULE_BASE } from '../base.js';

const GLOBAL_SCOPE = typeof window !== 'undefined' ? window : {};
const safeWarn = MODULE_BASE.safeWarn;
...
if (!DOCUMENT) {
  // If we can't find document, we stop here (but in ESM top-level return is not allowed, so we wrap remainder or just let it fail/warn?)
  // Actually, standard ESM executes immediately. If document is missing, it will error later if we don't guard.
  // But onboarding tour is UI only, so document will exist in browser.
}
...
const SAFE_LOCAL_STORAGE = getSafeLocalStorage();
...
export { moduleApi as cineFeaturesOnboardingTour };
