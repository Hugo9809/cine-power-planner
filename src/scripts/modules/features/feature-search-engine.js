...
(function () {
  ...
  function applySearchTokenSynonyms(tokens, addToken) {
    ...
    ensureSynonymGroup(
      ['iso', 'asa', 'ei', 'exposureindex', 'cameraiso', 'sensitivity', 'gain'],
      [
        'iso',
        'asa',
        'ei',
        ...
        'gain',
        'exposureindexvalue',
        'exposureindexsetting',
      ],
      [
        ['exposure', 'camera', 'sensor'],
        ['index', 'value', 'gain', 'sensitivity'],
      ],
    );

    ensureSynonymGroup(
      ['nd', 'neutraldensity', 'ndfilter', 'neutraldensityfilter', 'internalnd'],
      [
        'nd',
        'neutraldensity',
        'ndfilter',
        ...
        'internalnd',
        'internalneutraldensity',
      ],
      [
        ['neutral', 'internal'],
        ['density', 'densityfilter', 'filter'],
      ],
    );

    ensureSynonymGroup(
      ['whitebalance', 'wb', 'colortemperature', 'colourtemperature', 'kelvin'],
      ['whitebalance', 'wb', 'colortemperature', 'colourtemperature', 'kelvin'],
      [
        ['white'],
        ['balance'],
    ...
    ensureSynonymGroup(
      ['slowmotion', 'slowmo', 'highspeed', 'overcrank', 'highframerate'],
      [
        'slowmotion',
        'slowmo',
        'highspeed',
      ...
      [
        ['slow', 'high'],
        ['motion', 'speed', 'frame', 'framerate', 'fps'],
      ],
    );

    ensureSynonymGroup(
      ['timelapse', 'hyperlapse', 'intervalrecording', 'intervalshooting'],
      [
        'timelapse',
        'hyperlapse',
        'intervalrecording',
      ...
      [
        ['time', 'interval'],
        ['lapse', 'record', 'recording', 'shooting', 'capture'],
      ],
    );
  ...
  function getAdaptiveFuzzyMaxDistance(source, target, override) {
    ...
    const maxLength = Math.max(
      source && source.length ? source.length : 0,
      target && target.length ? target.length : 0,
    );
    if (maxLength <= 4) {
      return 2;
    }
  ...
  function createEngine(options) {
    ...
    const fuzzyMaxDistanceOverride = Number.isFinite(engineOptions.fuzzyMaxDistance)
    ...
    const getFuzzyDistance = (source, target) => {
      ...
      const maxDistance = getAdaptiveFuzzyMaxDistance(source, target, fuzzyMaxDistanceOverride);
    ...
    const computeFuzzyTokenScore = (token, entryToken) => {
      ...
      const maxDistance = getAdaptiveFuzzyMaxDistance(token, entryToken, fuzzyMaxDistanceOverride);
    ...
    const findBestSearchMatch = (map, key, tokens = []) => {
      ...
      for (const [entryKey, entryValue] of flattened) {
        ...
        if (hasKey && entryKey) {
          ...
          const maxDistance = getAdaptiveFuzzyMaxDistance(entryKey, key, fuzzyMaxDistanceOverride);
