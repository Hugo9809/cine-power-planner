/**
 * Cine Power Planner Auto Gear UI
 * ESM Module
 * 
 * Manages the Auto Gear UI state and DOM interactions.
 */
import { detectGlobalScope } from '../helpers/scope-utils.js';

export function initAutoGearUi(doc, options = {}) {
  const scope = options.globalScope || detectGlobalScope();
  const autoGearDocument = doc || (typeof document !== 'undefined' ? document : null);

  // Bind globals for easier access (read-only references or getters)
  // Note: Mutable globals like currentLang need to be accessed via scope directly 
  // or via a getter if we want to avoid prefixing everything. 
  // For safety/clarity we will use scope.X in the implementation where possible, 
  // or define local getters.
  ...
  if (!autoGearDocument) {
    ...
    function refreshAutoGearScenarioOptions(selected) {
      ...
      if (!hasOptions) {
        ...
        placeholder.value = '';
        placeholder.textContent = scope.texts[scope.currentLang]?.autoGearScenarioPlaceholder
    ...
    function applyAutoGearScenarioSettings(selectedValues) {
      ...
      if (autoGearScenarioModeSelectElement) {
        const modeLabels = {
          all: scope.texts[scope.currentLang]?.autoGearScenarioModeAll
            || texts.en?.autoGearScenarioModeAll
            || 'Require every selected scenario',
          any: scope.texts[scope.currentLang]?.autoGearScenarioModeAny
            || texts.en?.autoGearScenarioModeAny
            || 'Match any selected scenario',
          multiplier: scope.texts[scope.currentLang]?.autoGearScenarioModeMultiplier
    ...
    function refreshAutoGearScenarioBaseSelect(selectedValues, options = {}) {
      ...
      if (forceDisable || !uniqueValues.length) {
        const placeholder = autoGearDocument.createElement('option');
        placeholder.value = '';
        placeholder.textContent = scope.texts[scope.currentLang]?.autoGearScenarioBasePlaceholder
    ...
    function refreshAutoGearMatteboxOptions(selected) {
      ...
      if (!hasOptions) {
        const placeholder = autoGearDocument.createElement('option');
        placeholder.value = '';
        placeholder.textContent = scope.texts[scope.currentLang]?.autoGearMatteboxPlaceholder
    ...
    function refreshAutoGearCameraHandleOptions(selected) {
      ...
      if (!hasOptions) {
        const placeholder = autoGearDocument.createElement('option');
        placeholder.value = '';
        placeholder.textContent = scope.texts[scope.currentLang]?.autoGearCameraHandlePlaceholder
    ...
    function getViewfinderFallbackLabel(value) {
      if (value === '__none__') {
        return scope.texts[scope.currentLang]?.viewfinderExtensionNone
    ...
    function getVideoDistributionFallbackLabel(value) {
      if (value === '__none__') {
        return scope.texts[scope.currentLang]?.autoGearVideoDistributionNone
          || texts.en?.autoGearVideoDistributionNone
          || 'No video distribution selected';
      }
      const translated = scope.texts[scope.currentLang]?.videoDistributionOptions?.[value];
      if (translated) {
        return translated;
      }
      return value;
    ...
    function refreshAutoGearViewfinderExtensionOptions(selected) {
      ...
      if (!hasOptions) {
        const placeholder = autoGearDocument.createElement('option');
        placeholder.value = '';
        placeholder.textContent = scope.texts[scope.currentLang]?.autoGearViewfinderExtensionPlaceholder
    ...
    function refreshAutoGearDeliveryResolutionOptions(selected) {
      ...
      if (!autoGearDeliveryResolutionSelect.options.length) {
        const placeholder = autoGearDocument.createElement('option');
        placeholder.value = '';
        placeholder.textContent = scope.texts[scope.currentLang]?.autoGearDeliveryResolutionPlaceholder
    ...
    function refreshAutoGearVideoDistributionOptions(selected) {
      ...
      if (!hasOptions) {
        const placeholder = autoGearDocument.createElement('option');
        placeholder.value = '';
        placeholder.textContent = scope.texts[scope.currentLang]?.autoGearVideoDistributionPlaceholder
