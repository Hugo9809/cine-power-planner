...
function loaderCreateUnifyDevicesUtility() {
  function loaderNormalizeVideoTypeLocal(str, key) {
    ...
    if (k.indexOf('dp') === 0 && k.length === 2) return 'DisplayPort'; // strictly "dp" might be risky, but pattern said {needles:['dp']}
  ...
  function fixPowerInput(dev) {
    ...
    if (dev.powerInput && (!dev.power || !dev.power.input)) {
      dev.power = dev.power || {};
      dev.power.input = { type: normalizePowerPortType(dev.powerInput) };
      try { delete dev.powerInput; } catch (inputError) { void inputError; dev.powerInput = undefined; }
    }
  ...
  function unifyDevices(devicesData, options) {
    ...
    for (var ckIndex = 0; ckIndex < camKeys.length; ckIndex++) {
      ...
      if (cam.power && cam.power.input && typeof cam.power.input.powerDrawWatts !== 'undefined') {
        try { delete cam.power.input.powerDrawWatts; } catch (wattsError) { void wattsError; }
      }
      fixPowerInput(cam);
      ...
      if (cam.power && Array.isArray(cam.power.batteryPlateSupport)) {
        ...
        for (var b = 0; b < cam.power.batteryPlateSupport.length; b++) {
          ...
          if (typeof it === 'string') {
            ...
            bps.push({ type: type, mount: mount, notes: notes });
          } else {
      ...
      var videoOutputs = ensureList(cam.videoOutputs, { type: '', notes: '' });
      var newVideoOutputs = [];
      for (var v = 0; v < videoOutputs.length; v++) {
        ...
        if (Object.prototype.hasOwnProperty.call(VIDEO_OUTPUT_TYPES, norm) && VIDEO_OUTPUT_TYPES[norm]) {
          ...
          var base = {};
          for (var vok in vo) { if (vok !== 'count') base[vok] = vo[vok]; }
          base.type = norm;
          base.notes = vo.notes || '';
          for (var k = 0; k < num; k++) {
            var copy = {};
      ...
      var fizConnectors = ensureList(cam.fizConnectors, { type: '', notes: '' });
      ...
      var viewfinders = ensureList(cam.viewfinder, { type: '', resolution: '', connector: '', notes: '' });
      var newViewfinders = [];
      ...
      var recordingMedia = ensureList(cam.recordingMedia, { type: '', notes: '' });
      ...
      for (var rm = 0; rm < recordingMedia.length; rm++) {
        ...
        newRecordingMedia.push({ type: rType, notes: rNotes });
      }
      cam.recordingMedia = newRecordingMedia;

      cam.timecode = ensureList(cam.timecode, { type: '', notes: '' });

      var lensMount = ensureList(cam.lensMount, { type: '', mount: 'native', notes: '' });
      var newLensMount = [];
    ...
    for (var lKeyIndex = 0; lKeyIndex < lensKeys.length; lKeyIndex++) {
      ...
      var normalizeMountEntry = function (entry) {
        ...
        return { type: type, mount: status === 'adapted' ? 'adapted' : 'native' };
      };
      ...
      if (!normalizedOptions.length && Array.isArray(lens.lensMount)) {
        ...
        try { delete lens.lensMount; } catch (lensMountError) { void lensMountError; }
      }

      if (!normalizedOptions.length && typeof lens.mount === 'string') {
        var mt = lens.mount.trim();
        if (mt) {
          normalizedOptions.push({ type: mt, mount: 'native' });
        }
      }
      ...
      for (var no = 0; no < normalizedOptions.length; no++) {
        ...
        if (!found) dedupedOptions.push({ type: opt.type, mount: ms });
      }
      ...
      if (lens.mountOptions.length) {
        ...
        lens.mount = lens.mount.trim();
        if (!lens.mount) {
          try { delete lens.mount; } catch (mountDelError) { void mountDelError; }
        }
      }
    }
    ...
    for (var fmkI = 0; fmkI < fmkKeys.length; fmkI++) {
      ...
      if (fm.connector && !fm.fizConnector) {
        ...
        try { delete fm.connector; } catch (e) { void e; }
      }
      if (fm.fizConnector) {
    ...
    for (var fckI = 0; fckI < fckKeys.length; fckI++) {
      ...
      if (Array.isArray(c.fizConnectors)) {
        ...
        for (var pi = 0; pi < parts.length; pi++) {
          var s = parts[pi].trim();
          if (s) newCtrlFc2.push({ type: loaderNormalizeFizConnectorType(s) });
        }
        c.fizConnectors = newCtrlFc2;
        try { delete c.fizConnector; } catch (e) { void e; }
      } else {
        c.fizConnectors = [];
      }
    }
...
(function () {
  ...
  var modernScriptBundle = {
    core: [
      ...
      'src/scripts/modules/core/pink-mode.js?v=10',
      ...
      'src/scripts/app-core-runtime-global-tools.js',
      'src/scripts/app-core-ui-helpers.js',
      'src/scripts/app-core-runtime-ui.js',
      ...
      'src/scripts/legacy-globals-shim.js?v=10',
      'src/scripts/app-core-new-1.js?v=10',
      'src/scripts/app-core-new-2.js?v=10',

      'src/scripts/app-events.js?v=10',
      'src/scripts/app-setups.js?v=10',
      'src/scripts/app-session.js?v=10',
  ...
  var legacyScriptBundle = {
    core: [
      ...
      'legacy/scripts/modules/core/runtime.js',
      'legacy/scripts/modules/core/localization-runtime-environment.js',
      'legacy/scripts/modules/core/pink-mode.js',
      'legacy/scripts/modules/core/device-schema.js',
      'legacy/scripts/modules/core/project-intelligence.js',
      'legacy/scripts/modules/core/persistence-guard.js',
      'legacy/scripts/modules/core/mount-voltage.js',
      'legacy/scripts/modules/core/experience.js',
      'legacy/scripts/modules/logging.js',
      'legacy/scripts/modules/settings-and-appearance.js',
      'legacy/scripts/app-core-auto-gear-ui.js',
      ...
      'legacy/scripts/modules/app-core/bootstrap.js',
      'legacy/scripts/modules/app-core/pink-mode.js',
      'legacy/scripts/modules/app-core/localization.js',
      'legacy/scripts/modules/app-core/runtime.js',
      'legacy/scripts/modules/app-core/settings-documentation-tracker.js',
      ...
      'legacy/scripts/app-core-environment.js',
      'legacy/scripts/app-core-bootstrap.js',
      'legacy/scripts/app-core-runtime-shared.js',
      'legacy/scripts/app-core-pink-mode.js',
      'legacy/scripts/app-core-runtime-candidate-scopes.js',
      'legacy/scripts/app-core-runtime-global-tools.js',
