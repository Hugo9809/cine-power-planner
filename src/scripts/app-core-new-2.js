...
function normalizeLegacyAutoGearLabelText(text) {
...
function formatLegacyAutoGearQuantity(quantity) {
  const value = Math.abs(Math.round(Number(quantity) || 0));
  return value > 0 ? `${value}×` : '0×';
}

function getLegacyAutoGearLabelCandidates(rule) {
  const normalizedRule = normalizeAutoGearRule(rule);
  if (!normalizedRule) return [];
  const aggregates = new Map();
  normalizedRule.add.forEach(item => {
    if (!item || typeof item !== 'object') return;
    ...
    if (!aggregates.has(key)) {
      aggregates.set(key, { name, total: 0, contexts: new Map() });
    }
    const entry = aggregates.get(key);
    entry.total += quantity;
    const contexts = Array.isArray(item.contextNotes) ? item.contextNotes : [];
    contexts.forEach(note => {
      const trimmed = typeof note === 'string' ? note.trim() : '';
      if (!trimmed) return;
      entry.contexts.set(trimmed, (entry.contexts.get(trimmed) || 0) + quantity);
    });
  });
  const labels = [];
  aggregates.forEach(entry => {
    if (!entry.total) return;
    const totalLabel = formatLegacyAutoGearQuantity(entry.total);
    const contextParts = Array.from(entry.contexts.entries()).map(([note, count]) => `${formatLegacyAutoGearQuantity(count)} ${note}`);
    contextParts.sort((a, b) => a.localeCompare(b));
    const contextText = contextParts.length ? ` (${contextParts.join(', ')})` : '';
    labels.push(`${totalLabel} ${entry.name}${contextText}`);
  });
  if (labels.length > 1) {
    const combined = labels.slice().sort((a, b) => a.localeCompare(b)).join(' + ');
    labels.push(combined);
  }
  return labels;
}

function isLegacyAutoGeneratedAutoGearLabel(rule, label) {
  if (typeof label !== 'string' || !label.trim()) return false;
  if (label.indexOf('(') === -1 && label.indexOf('+') === -1) return false;
  const normalizedLabel = normalizeLegacyAutoGearLabelText(label);
  if (!normalizedLabel) return false;
  const candidates = getLegacyAutoGearLabelCandidates(rule);
  if (!candidates.length) return false;
  return candidates.some(candidate => normalizeLegacyAutoGearLabelText(candidate) === normalizedLabel);
}
...
function renderAutoGearRulesList() {
  ...
  visibleRules.forEach(rule => {
    ...
    const displayLabel = rawLabel && !isLegacyAutoGeneratedAutoGearLabel(rule, rawLabel)
      ? rawLabel
      : (fallbackTitle
        || langTexts.autoGearRuleBadgeUnnamed
        || texts.en?.autoGearRuleBadgeUnnamed
        || 'Automatic rule');
