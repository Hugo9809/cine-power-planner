...
if (CORE_PART2_RUNTIME_SCOPE && CORE_PART2_RUNTIME_SCOPE.__cineCorePart2Initialized) {
  ...
  function corePart2Runtime() {
    ...
    function createFeatureSearchEngineFallback() {
      ...
      const FILTER_STRIP_PATTERN = /^[\s:> /=\-?,.]+/;
      const normalizeQuotes = value =>
        typeof value === 'string' ? value.replace(/[“”„«»]/g, '"') : '';
      const extractQuoted = query => {
      ...
      const extractFilter = query => {
        ...
        const filterType = FILTER_ALIASES.get(alias) || null;
        if (!filterType) {
          return { filterType: null, queryText: trimmed };
        }
        ...
        const remainder = remainderRaw.replace(FILTER_STRIP_PATTERN, '').trim();
        return { filterType, queryText: remainder };
      };
      const createNormalizer = () => value =>
        typeof value === 'string' ? value.trim().toLowerCase() : '';
      const simpleSearchKey = value => {
        if (!value) return '';
        return String(value)
          .toLowerCase()
          .replace(/[^a-z0-9]+/g, '');
      };
      const simpleSearchTokens = value => {
        if (!value) return [];
        return String(value)
          .toLowerCase()
          .split(/[^a-z0-9]+/)
      ...
      const simpleScore = (entry, queryKey, queryTokens, rawQueryText, opts = {}) => {
        if (!entry || !entry.key) return null;
        const entryKey = entry.key;
        const entryTokens = Array.isArray(entry.tokens) ? entry.tokens : [];
        const primaryTokens = Array.isArray(entry.primaryTokens) ? entry.primaryTokens : entryTokens;
        const validTokens = Array.isArray(queryTokens) ? queryTokens.filter(Boolean) : [];
        const matchedTokens = validTokens.filter(token => entryTokens.includes(token));
        const primaryMatches = validTokens.filter(token => primaryTokens.includes(token));
        const normalizedQuery = normalizeString(rawQueryText).toLowerCase();
        let matchType = 'none';
        if (queryKey) {
          if (entryKey === queryKey) {
            matchType = 'exactKey';
          } else if (entryKey.startsWith(queryKey)) {
            matchType = 'keyPrefix';
          } else if (queryKey.startsWith(entryKey)) {
            matchType = 'keySubset';
          } else if (entryKey.includes(queryKey) || queryKey.includes(entryKey)) {
            matchType = 'partial';
          }
        }
        const baseScore = matchedTokens.length;
        const labelMatch = normalizedQuery && normalizeString(entry.display).toLowerCase().includes(normalizedQuery)
          ? normalizedQuery.length
          : 0;
        const historyLookup = typeof opts.getHistoryData === 'function' ? opts.getHistoryData : null;
        const history = historyLookup ? historyLookup(entryKey, entry.type || 'feature') : null;
        const historyCount = history && typeof history === 'object' ? Number(history.count) || 0 : 0;
        const historyLastUsed = history && typeof history === 'object' ? Number(history.lastUsed) || 0 : 0;
        const nowValue = typeof opts.now === 'number' ? opts.now : Date.now ? Date.now() : new Date().getTime();
        const historyScore = Math.min(Math.max(historyCount, 0), 5) * 2 + (historyLastUsed ? 1 : 0);
        const priority = MATCH_PRIORITIES[matchType] || MATCH_PRIORITIES.none;
        return {
          entry,
          entryType: entry.type || 'feature',
          typePriority: TYPE_PRIORITIES[entry.type || 'feature'] || 0,
          allTokensMatched: validTokens.length > 0 && matchedTokens.length >= validTokens.length,
          matchType,
          priority,
          tokenScore: baseScore,
          tokenMatches: matchedTokens.length,
          primaryTokenScore: primaryMatches.length,
          primaryTokenMatches: primaryMatches.length,
          phraseScore: labelMatch,
          phraseMatched: labelMatch > 0,
          fuzzyDistance: Number.POSITIVE_INFINITY,
          keyDistance: queryKey ? Math.abs(entryKey.length - queryKey.length) : Number.POSITIVE_INFINITY,
          keyLength: entryKey.length,
          historyCount,
          historyLastUsed,
          labelMatchLevel: labelMatch > 0 ? 1 : 0,
          labelMatchScore: labelMatch,
          historyBoostScore: historyScore,
          quotedPhraseScore: 0,
          quotedPhraseMatches: 0,
        };
      };
      const simpleCompare = (a, b) => {
        ...
        if (b.primaryTokenScore !== a.primaryTokenScore) {
          return b.primaryTokenScore - a.primaryTokenScore;
        }
        if (b.phraseScore !== a.phraseScore) {
          return b.phraseScore - a.phraseScore;
        }
        if (b.historyBoostScore !== a.historyBoostScore) {
          return b.historyBoostScore - a.historyBoostScore;
        }
      ...
      const simpleFindBest = (map, key, tokens = []) => {
        if (!map || typeof map.entries !== 'function') {
          return null;
        }
        const normalizedKey = simpleSearchKey(key);
        ...
        for (const [entryKey, entryValue] of map.entries()) {
          ...
          for (const value of values) {
            if (!value) continue;
            const candidateKey = entryKey || value.key || '';
            const entryTokens = value.tokens || [];
            const tokenMatches = normalizedTokens.filter(token => entryTokens.includes(token));
            let matchType = 'none';
            if (normalizedKey) {
              if (candidateKey === normalizedKey) {
                matchType = 'exactKey';
              } else if (candidateKey.startsWith(normalizedKey)) {
                matchType = 'keyPrefix';
              } else if (normalizedKey.startsWith(candidateKey)) {
                matchType = 'keySubset';
              } else if (candidateKey.includes(normalizedKey) || normalizedKey.includes(candidateKey)) {
                matchType = 'partial';
              }
            } else if (tokenMatches.length) {
              matchType = 'token';
            }
            const score = tokenMatches.length + (matchType === 'exactKey' ? 5 : 0);
            if (!best || score > best.score) {
              best = {
                ...
                matchType,
                score,
                matchedCount: tokenMatches.length,
              };
            }
          }
        }
      ...
      return {
        FEATURE_SEARCH_MATCH_PRIORITIES: MATCH_PRIORITIES,
        FEATURE_SEARCH_TYPE_PRIORITIES: TYPE_PRIORITIES,
        createDefaultSearchNormalizer: createNormalizer,
        normalizeFeatureSearchQuotes: normalizeQuotes,
        extractFeatureSearchQuotedPhrases: extractQuoted,
        extractFeatureSearchFilter: extractFilter,
        scoreFeatureSearchEntry: simpleScore,
        compareFeatureSearchCandidates: simpleCompare,
        searchKey: simpleSearchKey,
        searchTokens: simpleSearchTokens,
        findBestSearchMatch: simpleFindBest,
        normalizeSpellingVariants: value => (typeof value === 'string' ? value : ''),
        normaliseMarkVariants: value => (typeof value === 'string' ? value : ''),
      };
    }

    const featureSearchEngineFallbackApi = createFeatureSearchEngineFallback();
    ...
    function resolveFeatureSearchEngineModuleApi() {
      ...
      for (let index = 0; index < candidates.length; index += 1) {
        ...
        if (
          candidate
          && typeof candidate === 'object'
          && typeof candidate.searchKey === 'function'
        ) {
          resolvedApi = candidate;
          break;
        }
      ...
      const api = resolvedApi || featureSearchEngineFallbackApi;

      if (globalScope) {
        try {
          globalScope[FEATURE_SEARCH_ENGINE_MODULE_CACHE_KEY] = api;
        } catch (error) {
          void error;
        }
      }
    ...
    const fallbackFeatureSearchEngineModuleApi = featureSearchEngineFallbackApi;

    const callFeatureSearchEngine = (name, args, fallbackFn) => {
      if (featureSearchEngineModuleApi && typeof featureSearchEngineModuleApi[name] === 'function') {
        try {
          return featureSearchEngineModuleApi[name](...args);
        } catch (error) {
          if (typeof console !== 'undefined' && console && typeof console.warn === 'function') {
    ...
    const createDefaultSearchNormalizer = () =>
      callFeatureSearchEngine(
        'createDefaultSearchNormalizer',
        [],
        fallbackFeatureSearchEngineModuleApi.createDefaultSearchNormalizer,
      );
    ...
    const FEATURE_SEARCH_MATCH_PRIORITIES =
      (featureSearchEngineModuleApi && featureSearchEngineModuleApi.FEATURE_SEARCH_MATCH_PRIORITIES)
      || fallbackFeatureSearchEngineModuleApi.FEATURE_SEARCH_MATCH_PRIORITIES;

    const normalizeFeatureSearchQuotes = value =>
      callFeatureSearchEngine(
        'normalizeFeatureSearchQuotes',
        [value],
        fallbackFeatureSearchEngineModuleApi.normalizeFeatureSearchQuotes,
      );
    ...
    const extractFeatureSearchQuotedPhrases = query =>
      callFeatureSearchEngine(
        ...
        [query],
        fallbackFeatureSearchEngineModuleApi.extractFeatureSearchQuotedPhrases,
      );
    ...
    const extractFeatureSearchFilter = query =>
      callFeatureSearchEngine(
        ...
        [query],
        fallbackFeatureSearchEngineModuleApi.extractFeatureSearchFilter,
      );

    const searchKey = value =>
      callFeatureSearchEngine('searchKey', [value], fallbackFeatureSearchEngineModuleApi.searchKey);

    const searchTokens = value =>
      callFeatureSearchEngine('searchTokens', [value], fallbackFeatureSearchEngineModuleApi.searchTokens);

    const scoreFeatureSearchEntry = (entry, queryKey, queryTokens, rawQueryText, quotedPhrases = []) =>
      callFeatureSearchEngine(
        'scoreFeatureSearchEntry',
        [entry, queryKey, queryTokens, rawQueryText, { quotedPhrases, getHistoryData: getFeatureSearchHistoryData }],
        fallbackFeatureSearchEngineModuleApi.scoreFeatureSearchEntry,
      );
    ...
    const compareFeatureSearchCandidates = (a, b) =>
      callFeatureSearchEngine(
        ...
        [a, b],
        fallbackFeatureSearchEngineModuleApi.compareFeatureSearchCandidates,
      );
    ...
    const findBestSearchMatch = (map, key, tokens = []) =>
      callFeatureSearchEngine(
        'findBestSearchMatch',
        [map, key, tokens],
        fallbackFeatureSearchEngineModuleApi.findBestSearchMatch,
      );

    const normalizeSpellingVariants = value =>
      callFeatureSearchEngine(
        'normalizeSpellingVariants',
        [value],
        fallbackFeatureSearchEngineModuleApi.normalizeSpellingVariants,
      );

    const normaliseMarkVariants = value =>
      callFeatureSearchEngine(
        'normaliseMarkVariants',
        [value],
        fallbackFeatureSearchEngineModuleApi.normaliseMarkVariants,
      );
