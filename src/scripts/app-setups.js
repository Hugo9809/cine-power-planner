...
const lensSelectionManager = (() => {
    ...
    const getLensMountEntries = (lensName) => {
        const lensRecord = catalog.lensIndex.get(lensName);
        if (!lensRecord || typeof lensRecord !== 'object') {
            return [];
        }
        ...
        const pushEntry = (type, status = 'native') => {
            const normalizedType = typeof type === 'string' ? type.trim() : '';
            if (!normalizedType) {
                return;
            }
            const normalizedStatus = typeof status === 'string' && status.trim().toLowerCase() === 'adapted'
                ? 'adapted'
                : 'native';
            if (entryMap.has(normalizedType)) {
                const existing = entryMap.get(normalizedType);
                if (existing && existing.status !== 'native' && normalizedStatus === 'native') {
                    existing.status = 'native';
                }
                return;
            }
            const entry = { type: normalizedType, status: normalizedStatus };
            entryMap.set(normalizedType, entry);
            entryOrder.push(normalizedType);
        };

        const addFromRawValue = (rawValue, status = 'native') => {
            if (!rawValue) {
                return;
            }
            const labels = extractMountLabels(rawValue);
            if (Array.isArray(labels) && labels.length) {
                labels.forEach(label => pushEntry(label, status));
                return;
            }
            pushEntry(rawValue, status);
        };

        const rawMountOptions = lensRecord?.data?.mountOptions;
        if (Array.isArray(rawMountOptions) && rawMountOptions.length) {
            rawMountOptions.forEach(option => {
                if (!option) {
                    return;
                }
                const optionStatus = typeof option.mount === 'string' && option.mount.trim().toLowerCase() === 'adapted'
                    ? 'adapted'
                    : 'native';
        ...
        if (!entryOrder.length && Array.isArray(lensRecord.mountLabels) && lensRecord.mountLabels.length) {
            lensRecord.mountLabels.forEach(label => pushEntry(label, 'native'));
        }

        if (!entryOrder.length) {
            addFromRawValue(lensRecord?.data?.mount, 'native');
        }
    ...
    const getLensMountLabels = (lensName) => getLensMountEntries(lensName).map(entry => entry.type);
    ...
    const getCameraLensMountEntries = () => {
        ...
        const entryMap = new Map();
        const entryOrder = [];
        camera.lensMount.forEach(rawEntry => {
            if (!rawEntry) {
                return;
            }
            const normalizedType = getMountLabelFromCameraEntry(rawEntry);
            if (!normalizedType) {
                return;
            }
            const status = typeof rawEntry.mount === 'string' && rawEntry.mount.trim().toLowerCase() === 'adapted'
                ? 'adapted'
                : 'native';
            if (entryMap.has(normalizedType)) {
                const existing = entryMap.get(normalizedType);
            ...
            const entry = { type: normalizedType, status };
            entryMap.set(normalizedType, entry);
            entryOrder.push(normalizedType);
        });
    ...
    const getCameraNativeMount = () => {
        ...
        const nativeEntry = entries.find(entry => entry.status === 'native');
        if (nativeEntry) {
    ...
    const determineLensMountSelection = (lensName, preferredMount = '') => {
        const lensEntries = getLensMountEntries(lensName);
        const lensTypes = lensEntries.map(entry => entry.type);
        const cameraEntries = getCameraLensMountEntries();

        const resolveStatusForType = (type) => {
            if (!type) {
                return '';
            }
            const cameraMatch = cameraEntries.find(entry => entry.type === type && entry.status === 'native')
                || cameraEntries.find(entry => entry.type === type);
            if (cameraMatch) {
                return cameraMatch.status;
            }
            const lensMatch = lensEntries.find(entry => entry.type === type);
            return lensMatch ? lensMatch.status : '';
        };

        const normalizedPreferred = typeof preferredMount === 'string' ? preferredMount.trim() : '';
        ...
        const adaptedMatch = cameraEntries.find(entry => entry.status === 'adapted' && lensTypes.includes(entry.type));
        if (adaptedMatch) {
            return { mount: adaptedMatch.type, status: resolveStatusForType(adaptedMatch.type) };
        }

        if (lensEntries.length) {
            const fallbackType = lensEntries[0].type;
            const fallbackStatus = resolveStatusForType(fallbackType);
            const hasCameraEntries = cameraEntries.length > 0;
            const cameraMatchesFallback = cameraEntries.some(entry => entry.type === fallbackType);
            let inferredStatus = fallbackStatus;
            if (!cameraMatchesFallback && hasCameraEntries) {
                inferredStatus = 'adapted';
            } else if (!inferredStatus && hasCameraEntries) {
                inferredStatus = 'adapted';
            }
            return { mount: fallbackType, status: inferredStatus };
        }
    ...
    const renderSelectionChips = ({ skipEvent = false, skipDirty = false } = {}) => {
        ...
        state.selections.forEach((selection, index) => {
            ...
            const mountResolution = determineLensMountSelection(selection.name, storedMount);
            ...
            mountOptionsForSelect.forEach(label => {
                ...
                option.value = label;
                const optionResolution = determineLensMountSelection(selection.name, label);
                const optionStatus = optionResolution.status;
                option.textContent = optionStatus === 'adapted' ? `${label} (adapted)` : label;
                if (optionStatus) {
            ...
            if (!selection.mount && mountOptionsForSelect.length) {
                ...
                const fallbackResolution = determineLensMountSelection(selection.name, selection.mount);
            ...
            mountSelect.addEventListener('change', (event) => {
                ...
                const resolution = determineLensMountSelection(lensName, newMount);
                const resolvedMount = resolution.mount;
                const resolvedStatus = resolution.status;
                const previousStatus = record.mountState || '';
    ...
    const handleLensToggle = (lensName, checked) => {
        if (checked) {
            ...
            const defaultMount = determineLensMountSelection(lensName);
            const selection = {
                name: lensName,
    ...
    const applyInfo = (info = {}) => {
        ...
        if (Array.isArray(info.lensSelections)) {
            info.lensSelections.forEach(entry => {
                ...
                const mountState = typeof entry.mountState === 'string' ? entry.mountState.trim() : '';
                prepared.push({ name, mount, mountState });
                seen.add(name);
            });
        }
        if (!prepared.length) {
        ...
        state.selections = prepared.map(entry => {
            const preferredMount = typeof entry.mount === 'string' ? entry.mount.trim() : '';
            const resolution = determineLensMountSelection(entry.name, preferredMount);
            const selection = {
                name: entry.name,
                mount: resolution.mount
            };
            const resolvedState = resolution.status || (typeof entry.mountState === 'string' ? entry.mountState.trim() : '');
            if (resolvedState) {
    ...
    const snapshot = () => ({
        ...
        lensSelections: state.selections.map(selection => ({
            name: selection.name,
            mount: selection.mount || '',
            ...(selection.mountState ? { mountState: selection.mountState } : {})
        })),
...
function updateGearItemEditCameraLinkOptions(context, data = {}) {
  ...
  if (isCameraItem) {
    CAMERA_LINK_LETTERS.forEach(letter => {
      const option = doc.createElement('option');
      option.value = letter;
      const labelTemplate = texts.cameraLinkDefaultLabel || 'Camera %s';
      option.textContent = labelTemplate.replace('%s', letter);
      option.dataset.cameraLabel = labelTemplate.replace('%s', letter);
      select.appendChild(option);
    });
    const desiredLetter = normalizeCameraLetter(
      typeof data.cameraLink === 'string' && data.cameraLink
        ? data.cameraLink
  ...
  const cameraEntries = collectCameraSourceEntries();
  const entryMap = new Map();
  ...
  CAMERA_LINK_LETTERS.forEach(letter => {
    const option = doc.createElement('option');
    option.value = letter;
    const entry = entryMap.get(letter);
    const assignedTemplate = texts.cameraLinkCameraOption || 'Camera %s — %s';
    const unavailableTemplate = texts.cameraLinkUnavailableOption || 'Camera %s (unassigned)';
    const label = entry && entry.label
      ? assignedTemplate.replace('%s', letter).replace('%s', entry.label)
      : unavailableTemplate.replace('%s', letter);
    ...
    if (entry && entry.label) {
      ...
      const defaultLabelTemplate = texts.cameraLinkDefaultLabel || 'Camera %s';
  ...
  const desiredValue = normalizeCameraLetter(
    typeof data.cameraLink === 'string' && data.cameraLink
      ? data.cameraLink
      : context.currentCameraLinkValue || ''
  );
    ...
    const lensSelectionsDetailed = Array.isArray(lensSnapshot.lensSelections)
        ? lensSnapshot.lensSelections
            .map(entry => ({
                ...
                mountState: typeof entry?.mountState === 'string' ? entry.mountState.trim() : ''
            }))
...
let cachedCameraColorDefaults = null;
...
function getCameraLetterColorDefaults() {
  ...
  if (storedPalette && typeof storedPalette === 'object') {
    CAMERA_LINK_LETTERS.forEach(letter => {
      const incoming = storedPalette[letter] || storedPalette[letter.toLowerCase()];
      const normalized = normalizeCameraLinkColorValue(incoming);
      if (normalized) {
...
function getCameraLetterColorsSafe() {
  const defaults = getCameraLetterColorDefaults();
  if (typeof getCameraLetterColors === 'function') {
    try {
      const provided = getCameraLetterColors();
      if (provided && typeof provided === 'object') {
        const normalized = { ...defaults };
        CAMERA_LINK_LETTERS.forEach(letter => {
          const value = provided[letter] || provided[letter.toLowerCase()];
          const normalizedValue = normalizeCameraLinkColorValue(value);
          if (normalizedValue) {
...
function getCameraLetterColor(letter) {
  const colors = getCameraLetterColorsSafe();
  const key = normalizeCameraLetter(letter);
  if (key && colors[key]) {
    return colors[key];
  }
  return colors.A;
...
function collectCameraSourceEntries(root) {
  const scope = root || gearListOutput || (typeof document !== 'undefined' ? document : null);
  if (!scope || typeof scope.querySelectorAll !== 'function') {
    return [];
  }
  const nodes = scope.querySelectorAll('[data-gear-camera-role="source"], [data-gear-camera-letter]');
  const seen = new Map();
  nodes.forEach(node => {
    ...
    const letterAttr = node.getAttribute('data-gear-camera-letter')
      || node.getAttribute('data-gear-camera-link')
      || '';
    const letter = normalizeCameraLetter(letterAttr);
    if (!letter || seen.has(letter)) {
      return;
    }
    const label = (node.getAttribute('data-gear-camera-link-label')
      || node.getAttribute('data-gear-label')
      || node.textContent
      || '').trim();
    seen.set(letter, {
      letter,
...
function refreshAllCameraLinkIndicators(scope) {
  const root = scope || gearListOutput || (typeof document !== 'undefined' ? document : null);
  if (!root || typeof root.querySelectorAll !== 'function') {
    return;
  }
  const nodes = root.querySelectorAll('.gear-item, .gear-custom-item');
  ...
  nodes.forEach(element => {
    ...
    const letterAttr = element.getAttribute('data-gear-camera-link')
      || element.getAttribute('data-gear-camera-letter')
      || '';
    const letter = normalizeCameraLetter(letterAttr);
    const cameraItem = isPrimaryCameraItem(element);
    const label = element.getAttribute('data-gear-camera-link-label')
      || (cameraItem ? getActiveCameraDisplayName() : '')
      || '';
    const badgeLabel = label ? `${badgeBase} — ${label}` : badgeBase;
    const color = letter ? getCameraLetterColor(letter) : '';
...
function updateGearItemCameraLinkIndicator(element, active, options = {}) {
  ...
  if (icon) {
    const letter = normalizeCameraLetter(options.letter || options.iconText || '') || 'A';
    icon.textContent = letter;
    indicator.setAttribute('data-camera-letter', letter);
    const color = options.color || getCameraLetterColor(letter);
    if (color) {
      indicator.style.setProperty('--camera-link-color', color);
      const textColor = computeCameraLetterTextColor(color);
      if (textColor) {
...
function getGearItemData(element) {
  ...
  const cameraLetterAttr = element.getAttribute('data-gear-camera-letter') || '';
  const cameraLinkLabelAttr = element.getAttribute('data-gear-camera-link-label') || '';
  const cameraItem = isPrimaryCameraItem(element);
  let cameraLinkValue = normalizeCameraLetter(cameraLinkAttr || cameraLetterAttr);
...
function applyGearItemData(element, data = {}, options = {}) {
  ...
  const cameraLinkRaw = normalizeCameraLetter(typeof data.cameraLink === 'string' ? data.cameraLink.trim() : '');
  const cameraLabelRaw = typeof data.cameraLinkLabel === 'string' ? data.cameraLinkLabel.trim() : '';
  const cameraItem = isPrimaryCameraItem(element);
  const effectiveLetter = cameraItem && !cameraLinkRaw ? 'A' : cameraLinkRaw;
  const wantsCameraLink = Boolean(effectiveLetter);
  ...
  const badgeLabel = effectiveCameraLabel
    ...
    : badgeBase;
  const indicatorLetter = wantsCameraLink ? effectiveLetter : '';
  const indicatorColor = indicatorLetter ? getCameraLetterColor(indicatorLetter) : '';
...
function getGearItemEditTexts() {
  ...
  return {
    ...
    cameraAssignmentLabel: langTexts.gearListEditCameraAssignmentLabel
      || fallbackTexts.gearListEditCameraAssignmentLabel
      || 'Camera designation',
    cameraAssignmentHelp: langTexts.gearListEditCameraAssignmentHelp
      || fallbackTexts.gearListEditCameraAssignmentHelp
      || '',
    ...
    cameraLinkCameraOption: langTexts.gearListEditCameraLinkCameraOption
      || fallbackTexts.gearListEditCameraLinkCameraOption
      || 'Camera %s — %s',
    cameraLinkUnavailableOption: langTexts.gearListEditCameraLinkUnavailableOption
      || fallbackTexts.gearListEditCameraLinkUnavailableOption
      || 'Camera %s (unassigned)',
    cameraLinkDefaultLabel: langTexts.gearListEditCameraLinkDefaultLabel
...
function handleGearItemEditFormSubmit(event) {
  ...
  const desiredLetter = normalizeCameraLetter(
    isCameraItem && !cameraLinkSelection ? 'A' : cameraLinkSelection
  );
  const wantsCameraLink = Boolean(desiredLetter);
  if (wantsCameraLink) {
    data.cameraLink = desiredLetter;
    data.cameraLinkLabel = cameraLinkLabel
      || (data.name && data.name.trim())
      || getActiveCameraDisplayName()
      || '';
  } else {
    data.cameraLink = '';
...
function refreshGearItemEditCameraLinkOptionsIfOpen() {
  ...
  if (context.cameraLinkLabel || context.cameraLinkHelp) {
    const textsForDialog = getGearItemEditTexts();
    if (context.cameraLinkLabel) {
      const labelText = isCameraItem
        ? textsForDialog.cameraAssignmentLabel
        : textsForDialog.cameraLinkLabel;
      context.cameraLinkLabel.textContent = labelText;
      const labelHelp = isCameraItem
        ? textsForDialog.cameraAssignmentHelp
        : textsForDialog.cameraLinkHelp;
      if (labelHelp) {
  ...
  const nextValue = isCameraItem
    ? (normalizeCameraLetter(data.cameraLink) || 'A')
    : normalizeCameraLetter(data.cameraLink);
...
function openGearItemEditor(element, options = {}) {
  ...
  const cameraLetter = normalizeCameraLetter(data.cameraLink);
...
const gearCartCatalogHelpers = (() => {
    const CART_LEGACY_HINTS = [
        ...
        { pattern: /magliner\s+\d+\s*"?\s*cinema/, value: 'Backstage Magliner Junior 24" Film Cart (8" Conversion Kit)' },
        { pattern: /magliner\s+junior\s+\d+\s*"/, value: 'Backstage Magliner Junior 24" Film Cart (8" Conversion Kit)' },
        { pattern: /ranger\s*36/, value: 'Inovativ Ranger 36 Equipment Cart' },
        { pattern: /ranger\s*48/, value: 'Inovativ Ranger 48 Equipment Cart' },
        ...
        { pattern: /apollo\s*40/, value: 'Inovativ Apollo 40 Equipment Cart' },
        { pattern: /apollo\s*52/, value: 'Inovativ Apollo 52 Equipment Cart' },
    ...
    function resolveCartDatasetName(rawName, cartDatabase, cartDatabaseNames) {
        if (!Array.isArray(cartDatabaseNames) || !cartDatabaseNames.length) {
            return '';
        }
        const trimmed = typeof rawName === 'string' ? rawName.trim() : '';
        if (trimmed && cartDatabase && Object.prototype.hasOwnProperty.call(cartDatabase, trimmed)) {
            return trimmed;
        }
    ...
    return {
        resolveCartDatasetName,
        buildCartConfigurationOptions,
        buildCartAccessoriesOptions,
        buildCartOptionBlock,
    };
})();
...
function gearListGenerateHtmlImpl(info = {}) {
    ...
    function buildCartItemHtml(rawName, index) {
      ...
      const resolved = gearCartCatalogHelpers.resolveCartDatasetName(rawName, cartDatabase, cartDatabaseNames)
        || DEFAULT_CART_NAME
        || '';
      if (!resolved && !rawName) {
        return '';
      ...
      const configOptions = gearCartCatalogHelpers.buildCartConfigurationOptions(cartEntry);
      const accessoriesOptions = gearCartCatalogHelpers.buildCartAccessoriesOptions(cartEntry);
      const configBlock = configOptions.length
        ? gearCartCatalogHelpers.buildCartOptionBlock(`gearListCartConfig${index}`, resolveGearListCustomText('gearListCartConfigurationLabel', 'Cart configuration'), configOptions)
        : '';
      const accessoriesBlock = accessoriesOptions.length
        ? gearCartCatalogHelpers.buildCartOptionBlock(`gearListCartAccessories${index}`, resolveGearListCustomText('gearListCartAccessoriesLabel', 'Cart accessories'), accessoriesOptions)
        : '';
...
function getCartDeviceDatabase() {
    return typeof devices === 'object' && devices && devices.carts && typeof devices.carts === 'object'
        ? devices.carts
        : {};
}

function syncGearListCartEntry(select) {
    if (!select || !gearCartCatalogHelpers) return;
    const gearItem = select.closest('.gear-item[data-gear-cart-entry]');
    if (!gearItem) return;
    const cartDatabase = getCartDeviceDatabase();
    const cartDatabaseNames = Object.keys(cartDatabase)
        .filter(name => name && name !== 'None');
    const cartLabel = resolveGearListCustomText('gearListCartSelectorLabel', 'Camera cart');
    const selectedValue = typeof select.value === 'string' ? select.value.trim() : '';
    const resolvedName = gearCartCatalogHelpers.resolveCartDatasetName(selectedValue, cartDatabase, cartDatabaseNames)
        || selectedValue
        || '';
    const cartEntry = (resolvedName && cartDatabase[resolvedName]) || (selectedValue && cartDatabase[selectedValue]) || null;
    const idMatch = select.id && select.id.match(/^gearListCartSelect(.+)$/);
    const idSuffix = idMatch && idMatch[1] ? idMatch[1] : (select.id || '');
    const configOptions = gearCartCatalogHelpers.buildCartConfigurationOptions(cartEntry);
    const accessoriesOptions = gearCartCatalogHelpers.buildCartAccessoriesOptions(cartEntry);
    const configBlock = configOptions.length
        ? gearCartCatalogHelpers.buildCartOptionBlock(
            `gearListCartConfig${idSuffix}`,
            resolveGearListCustomText('gearListCartConfigurationLabel', 'Cart configuration'),
            configOptions
        )
        : '';
    const accessoriesBlock = accessoriesOptions.length
        ? gearCartCatalogHelpers.buildCartOptionBlock(
            `gearListCartAccessories${idSuffix}`,
            resolveGearListCustomText('gearListCartAccessoriesLabel', 'Cart accessories'),
            accessoriesOptions
        )
        : '';
    const detailHtml = [configBlock, accessoriesBlock].filter(Boolean).join('');
    let detailContainer = gearItem.querySelector('.gear-cart-detail');
    if (detailHtml) {
        if (!detailContainer) {
            detailContainer = document.createElement('div');
            detailContainer.className = 'gear-cart-detail';
            const parent = select.closest('.gear-item-text') || gearItem;
            if (parent && typeof select.insertAdjacentElement === 'function') {
                select.insertAdjacentElement('afterend', detailContainer);
            } else {
    ...
    const attributeText = resolvedName ? (addArriKNumber(resolvedName) || resolvedName) : '';
    const labelText = cartLabel || 'Camera cart';
    const nameText = attributeText ? `${labelText} (${attributeText})` : labelText;
    if (nameText) {
        gearItem.setAttribute('data-gear-name', nameText);
        gearItem.setAttribute('data-gear-label', labelText);
    } else {
        gearItem.removeAttribute('data-gear-name');
        gearItem.removeAttribute('data-gear-label');
    }
    if (attributeText) {
        gearItem.setAttribute('data-gear-attributes', attributeText);
    } else {
        gearItem.removeAttribute('data-gear-attributes');
    }
}

function bindGearListCartListener() {
    if (!gearListOutput) return;
    const selects = gearListOutput.querySelectorAll('select[id^="gearListCartSelect"]');
    if (!selects.length) return;
    selects.forEach(select => {
        if (select._gearCartListenerBound) {
            syncGearListCartEntry(select);
            return;
        }
        ...
        select._gearCartListenerBound = true;
        syncGearListCartEntry(select);
    });
}
...
if (typeof document !== 'undefined') {
  ...
  document.addEventListener('camera-colors-changed', () => {
