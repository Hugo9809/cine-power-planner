...
activates new versions automatically while leaving the optional **Force reload**
control available when you want to clear caches on your schedule. Saving,
sharing, import, backup and restore tools are always available and always run
locally so user data stays safe.
...
4. Load the planner once, close the tab, disconnect from the network (or toggle
   ...
   or **"{count} backups saved in the local vault."** beside the **Open local
   backup vault** button, open the vault, download every queued archive and make
   sure the banner disappears before you remain offline.
