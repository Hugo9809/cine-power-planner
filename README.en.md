...
4. Load the planner once, close the tab, disconnect from the network (or toggle
   ...
   assets.
5. Open **Help → Quick start checklist** and launch the guided tutorial. The new
   step navigator lets you revisit finished workflows without restarting, and
   the **Resume guided tutorial** button appears automatically if you paused
   mid-tour so offline progress is never lost.
...
> **Catalog update.** The FIZ hand unit catalog now lists Tilta's Nucleus-M II controller alongside fresh profiles for Preston's HU4, cmotion's cPRO hand unit, Teradek's CTRL.3, and DJI's Focus Pro hand unit so offline planners can compare more ecosystems without leaving the app.
