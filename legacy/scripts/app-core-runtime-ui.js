...
/*
 * Legacy Cine Power Planner runtime UI bridge.
 *
 * Mirrors the modern runtime bridge so both bundles share the same lookup and
 * caching logic for UI helpers. Keeping the helpers in sync ensures the legacy
 * offline experience keeps rendering buttons safely without duplicating lookup
 * code.
 */

(function initialiseLegacyCineCoreRuntimeUiBridge() {
  ...
  var primaryScope = candidateScopes.length > 0 ? candidateScopes[0] : null;
