...
function updateGearItemEditCameraLinkOptions(context) {
  ...
  var identityInfo = findCameraIdentityForLabel(cameraName);
  ...
  if (optionLabel.includes('%s')) {
    var identityLabel = identityInfo ? identityInfo.label : '';
    var combinedLabel = identityLabel && cameraName ? "".concat(identityLabel, " — ").concat(cameraName) : identityLabel || cameraName;
    if (combinedLabel) {
...
function updateGearItemEditCameraIdentityOptions(context) {
  var data = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : {};
  if (!context || !context.cameraIdentitySelect) return;
  var select = context.cameraIdentitySelect;
  while (select.firstChild) {
    select.removeChild(select.firstChild);
  }
  var doc = select.ownerDocument || (typeof document !== 'undefined' ? document : null);
  if (!doc) return;
  var texts = getGearItemEditTexts();
  CAMERA_IDENTITY_PRESETS.forEach(function (preset) {
    var option = doc.createElement('option');
    option.value = preset.id;
    var labelKey = CAMERA_IDENTITY_TRANSLATION_KEYS[preset.id];
    var labelText = labelKey && texts[labelKey] ? texts[labelKey] : 'Camera ' + preset.letter;
  ...
  var rawValue = data && Object.prototype.hasOwnProperty.call(data, 'cameraIdentity') ? data.cameraIdentity : context.currentCameraIdentity;
  var desired = typeof rawValue === 'string' && rawValue.trim() ? rawValue.trim() : 'camera-a';
  var hasDesired = Array.prototype.some.call(select.options, function (option) {
...
var CAMERA_IDENTITY_TRANSLATION_KEYS = {};
CAMERA_IDENTITY_PRESETS.forEach(function (preset) {
  CAMERA_IDENTITY_TRANSLATION_KEYS[preset.id] = preset.translationKey;
});
function getCameraIdentityPresetById(identityId) {
  var normalized = typeof identityId === 'string' ? identityId.trim().toLowerCase() : '';
  for (var i = 0; i < CAMERA_IDENTITY_PRESETS.length; i += 1) {
    if (CAMERA_IDENTITY_PRESETS[i].id === normalized) {
      return CAMERA_IDENTITY_PRESETS[i];
    }
  }
  return CAMERA_IDENTITY_PRESETS[0];
}
function getCameraIdentityLabel(identityId) {
  var preset = getCameraIdentityPresetById(identityId);
  var textsForDialog = getGearItemEditTexts();
  var key = CAMERA_IDENTITY_TRANSLATION_KEYS[preset.id];
  if (key && Object.prototype.hasOwnProperty.call(textsForDialog, key)) {
    var textValue = textsForDialog[key];
...
function hslToHex(h, s, l) {
  ...
  var saturation = Number.isFinite(s) ? Math.max(0, Math.min(1, s)) : 0;
  var lightness = Number.isFinite(l) ? Math.max(0, Math.min(1, l)) : 0;
  if (saturation === 0) {
    var gray = Math.round(lightness * 255);
    var hex = gray.toString(16).padStart(2, '0');
    return '#' + hex + hex + hex;
  }
...
function getReadableCameraTextColor(color) {
  ...
  var toLinear = function toLinear(value) {
    var srgb = value / 255;
...
function getCameraIdentityInfo(identityId, options) {
  var opts = options || {};
  var preset = getCameraIdentityPresetById(identityId);
  var overrideColor = normalizeHexColor(opts.colorOverride);
  var fallbackColor = normalizeHexColor(opts.fallbackColor);
  var color = overrideColor;
  if (!color) {
    if (preset.random) {
      color = fallbackColor;
      if (!color && opts.generateRandom) {
  ...
  var label = getCameraIdentityLabel(preset.id);
  var textColor = color ? getReadableCameraTextColor(color) : '';
  return {
    id: preset.id,
...
function getCameraItemElements() {
  ...
  var selector = '.gear-item[data-gear-camera-identity], .gear-custom-item[data-gear-camera-identity]';
  return Array.prototype.slice.call(gearListOutput.querySelectorAll(selector));
}
function findCameraIdentityForLabel(label) {
  var normalizedLabel = typeof label === 'string' ? label.trim().toLowerCase() : '';
  var items = getCameraItemElements();
  if (!items.length) {
    return getCameraIdentityInfo('camera-a');
  }
  var fallbackInfo = null;
  items.forEach(function (item) {
    var identityAttr = item.getAttribute('data-gear-camera-identity') || '';
    var colorAttr = item.getAttribute('data-gear-camera-color') || '';
    var info = getCameraIdentityInfo(identityAttr || 'camera-a', {
    ...
    var linkLabel = (item.getAttribute('data-gear-camera-link-label') || '').trim().toLowerCase();
...
function refreshLinkedCameraItems(changedCameraElement) {
  if (!gearListOutput) return;
  var cameraLabel = changedCameraElement ? String(changedCameraElement.getAttribute('data-gear-camera-link-label') || '') : '';
  var identityInfo = findCameraIdentityForLabel(cameraLabel);
  var items = gearListOutput.querySelectorAll('.gear-item[data-gear-camera-link="camera"], .gear-custom-item[data-gear-camera-link="camera"]');
  Array.prototype.forEach.call(items, function (item) {
    if (item === changedCameraElement) {
      return;
    }
    var linkedLabel = String(item.getAttribute('data-gear-camera-link-label') || '');
    ...
    var textsForDialog = getGearItemEditTexts();
    var baseLabel = textsForDialog.cameraLinkBadgeLabel || 'Linked to camera';
    var badgeLabel = linkedLabel ? baseLabel + ' — ' + linkedLabel : baseLabel;
...
function updateGearItemCameraLinkIndicator(element, active) {
  ...
  var colorValue = normalizeHexColor(options.identityColor);
  var contrastValue = options.identityTextColor || (colorValue ? getReadableCameraTextColor(colorValue) : '');
  if (element && element.style) {
    if (colorValue) {
...
function getGearItemData(element) {
  ...
  var cameraIdentityAttr = element.getAttribute('data-gear-camera-identity') || '';
  var cameraIdentityColorAttr = element.getAttribute('data-gear-camera-color') || '';
  var cameraLinkIdentityAttr = element.getAttribute('data-gear-camera-link-identity') || '';
  var cameraLinkColorAttr = element.getAttribute('data-gear-camera-link-color') || '';
...
function applyGearItemData(element) {
  ...
  var cameraIdentityRaw = typeof data.cameraIdentity === 'string' ? data.cameraIdentity.trim() : '';
  var cameraIdentityColorRaw = typeof data.cameraIdentityColor === 'string' ? data.cameraIdentityColor.trim() : '';
  var cameraLinkIdentityRaw = typeof data.cameraLinkIdentity === 'string' ? data.cameraLinkIdentity.trim() : '';
  var cameraLinkColorRaw = typeof data.cameraLinkColor === 'string' ? data.cameraLinkColor.trim() : '';
  var cameraItem = isPrimaryCameraItem(element);
  ...
  if (cameraItem) {
    var previousIdentity = element.getAttribute('data-gear-camera-identity') || '';
    var previousColor = element.getAttribute('data-gear-camera-color') || '';
    var identityId = cameraIdentityRaw || previousIdentity || 'camera-a';
...
function applyGearItemEditDialogTexts(context) {
  ...
  if (context.cameraIdentityHelp) {
    var identityHelp = textsForDialog.cameraIdentityHelp || '';
...
function handleGearItemEditFieldInput() {
  ...
  if (context.cameraIdentitySelect) {
    var _context$cameraIdentit;
    context.currentCameraIdentity = ((_context$cameraIdentit = context.cameraIdentitySelect.value) !== null && _context$cameraIdentit !== void 0 ? _context$cameraIdentit : '').trim();
  }
...
function handleGearItemEditFormSubmit(event) {
  ...
  if (context.cameraLinkSelect) {
    ...
    if (selectedOption) {
      ...
      cameraLinkIdentity = selectedOption.dataset ? selectedOption.dataset.cameraIdentity || '' : selectedOption.getAttribute('data-camera-identity') || '';
      cameraLinkColor = selectedOption.dataset ? selectedOption.dataset.cameraColor || '' : selectedOption.getAttribute('data-camera-color') || '';
      if (!cameraLinkLabel) {
  ...
  var identitySelect = context.cameraIdentitySelect;
  if (isCameraItem && identitySelect) {
    var identityOption = identitySelect.selectedOptions && identitySelect.selectedOptions[0];
    var optionColor = identityOption ? identityOption.dataset && identityOption.dataset.identityColor || identityOption.getAttribute('data-identity-color') || '' : '';
    var previousIdentity = targetEntry ? targetEntry.getAttribute('data-gear-camera-identity') || '' : '';
    var previousColor = targetEntry ? targetEntry.getAttribute('data-gear-camera-color') || '' : '';
    var desiredIdentity = identitySelect.value || previousIdentity || 'camera-a';
    var identityInfo = getCameraIdentityInfo(desiredIdentity, {
      colorOverride: optionColor || data.cameraIdentityColor || '',
      fallbackColor: optionColor || previousColor,
      generateRandom: true
    });
    data.cameraIdentity = identityInfo.id;
    data.cameraIdentityColor = identityInfo.random ? identityInfo.color || previousColor || '' : identityInfo.color || '';
  }
...
function openGearItemEditor(element) {
  ...
  if (context.cameraIdentityContainer) {
    if (isCameraItem) {
      ...
      if (context.cameraIdentitySelect) {
        var identityAttr = element.getAttribute('data-gear-camera-identity') || '';
        var desiredIdentity = data.cameraIdentity && data.cameraIdentity.trim() ? data.cameraIdentity.trim() : identityAttr || 'camera-a';
        var hasOption = Array.prototype.some.call(context.cameraIdentitySelect.options, function (option) {
          return option.value === desiredIdentity;
        });
