      ...
      (function parallelLoaderIterator(currentIndex) {
        function attachParallelScript() {
          ...
          script.onerror = function (event) {
            ...
            resetScriptRetryState(currentUrl);
            safeConsole.error('Failed to load parallel script:', currentUrl, '→', resolvedUrl, event && event.error);
            handleError({
              event: event,
              url: currentUrl,
              index: currentIndex
  ...
  function loadScriptsSequentially(items, options) {
    ...
    function loadScriptAtIndex(currentIndex) {
      ...
      script.onerror = function (event) {
        ...
        resetScriptRetryState(currentUrl);
        safeConsole.error('Failed to load script:', currentUrl, '→', resolvedUrl, event && event.error);
        var shouldAbort = false;
        if (typeof settings.onError === 'function') {
          try {
