...
var AUTO_GEAR_NORMALIZER_SCOPE = typeof globalThis !== 'undefined' && globalThis || typeof window !== 'undefined' && window || typeof self !== 'undefined' && self || typeof global !== 'undefined' && global || {};
function resolveAutoGearDefaultLanguageSource() {
  var candidateScopes = [AUTO_GEAR_NORMALIZER_SCOPE, AUTO_GEAR_NORMALIZER_SCOPE && AUTO_GEAR_NORMALIZER_SCOPE.global, typeof globalThis !== 'undefined' ? globalThis : null, typeof window !== 'undefined' ? window : null, typeof self !== 'undefined' ? self : null, typeof global !== 'undefined' ? global : null];
  for (var index = 0; index < candidateScopes.length; index += 1) {
    var scope = candidateScopes[index];
    if (!scope || _typeof(scope) !== 'object' && typeof scope !== 'function') continue;
    if (typeof scope.DEFAULT_LANGUAGE_SAFE === 'string' && scope.DEFAULT_LANGUAGE_SAFE) {
      return scope.DEFAULT_LANGUAGE_SAFE;
    }
    if (typeof scope.CPP_DEFAULT_LANGUAGE_SAFE === 'string' && scope.CPP_DEFAULT_LANGUAGE_SAFE) {
      return scope.CPP_DEFAULT_LANGUAGE_SAFE;
    }
    if (scope.cineCoreShared && typeof scope.cineCoreShared.DEFAULT_LANGUAGE_SAFE === 'string' && scope.cineCoreShared.DEFAULT_LANGUAGE_SAFE) {
...
function assignAutoGearLanguageFallback(scope, fallbackLanguage) {
  if (!scope || _typeof(scope) !== 'object' && typeof scope !== 'function') return;
  var assignIfMissing = function assignIfMissing(key) {
...
function getAutoGearFallbackTexts() {
  ...
  try {
    ...
    if (fallbackTexts && _typeof(fallbackTexts) === 'object') {
...
var AUTO_GEAR_DEFAULT_SELECTOR_TYPES = Object.freeze(['none', 'monitor', 'directorMonitor', 'tripodHeadBrand', 'tripodBowl', 'tripodTypes', 'tripodSpreader', 'fizHandUnit']);
function ensureAutoGearSelectorTypeGlobals(scope) {
  var existingMap = scope && scope.AUTO_GEAR_SELECTOR_TYPE_MAP && _typeof(scope.AUTO_GEAR_SELECTOR_TYPE_MAP) === 'object' ? scope.AUTO_GEAR_SELECTOR_TYPE_MAP : null;
  var resolvedMap = existingMap && Object.keys(existingMap).length ? existingMap : AUTO_GEAR_DEFAULT_SELECTOR_TYPES.reduce(function (acc, type) {
    acc[type.toLowerCase()] = type;
    return acc;
  }, Object.create(null));
  var existingSet = scope && scope.AUTO_GEAR_SELECTOR_TYPE_SET instanceof Set ? scope.AUTO_GEAR_SELECTOR_TYPE_SET : null;
  var resolvedSet = existingSet && existingSet.size ? existingSet : new Set(Object.keys(resolvedMap));
...
function normalizeAutoGearText(value, _a) {
  var _b = _a === void 0 ? {} : _a,
    _c = _b.collapseWhitespace,
    collapseWhitespace = _c === void 0 ? true : _c;
...
function normalizeAutoGearSelectorType(value) {
  ...
  var selectorSet = typeof AUTO_GEAR_SELECTOR_TYPE_SET !== 'undefined' && AUTO_GEAR_SELECTOR_TYPE_SET || AUTO_GEAR_SELECTOR_TYPE_SET_FALLBACK;
  var selectorMap = typeof AUTO_GEAR_SELECTOR_TYPE_MAP !== 'undefined' && AUTO_GEAR_SELECTOR_TYPE_MAP || AUTO_GEAR_SELECTOR_TYPE_MAP_FALLBACK;
...
function updateGlobalDevicesReference(nextDevices) {
  ...
  if (globalCoreShared && _typeof(globalCoreShared) === 'object') {
    try {
...
function resolveAutoGearCoreShared() {
  ...
  for (var index = 0; index < candidateScopes.length; index += 1) {
    var scope = candidateScopes[index];
    if (!scope || _typeof(scope) !== 'object' && typeof scope !== 'function') continue;
    if (scope.CORE_SHARED && _typeof(scope.CORE_SHARED) === 'object') {
...
function getAutoGearSelectorOptions(type, itemOrContext) {
  ...
  if (normalizedType === 'fizHandUnit') {
    var groups = (typeof AUTO_GEAR_HAND_UNIT_COMPATIBILITY_GROUPS === "undefined" ? "undefined" : _typeof(AUTO_GEAR_HAND_UNIT_COMPATIBILITY_GROUPS)) === 'object' ? AUTO_GEAR_HAND_UNIT_COMPATIBILITY_GROUPS : typeof globalThis !== 'undefined' && _typeof(globalThis.AUTO_GEAR_HAND_UNIT_COMPATIBILITY_GROUPS) === 'object' ? globalThis.AUTO_GEAR_HAND_UNIT_COMPATIBILITY_GROUPS : Object.create(null);
    var motorMap = (typeof AUTO_GEAR_HAND_UNIT_MOTOR_TO_GROUP === "undefined" ? "undefined" : _typeof(AUTO_GEAR_HAND_UNIT_MOTOR_TO_GROUP)) === 'object' ? AUTO_GEAR_HAND_UNIT_MOTOR_TO_GROUP : typeof globalThis !== 'undefined' && _typeof(globalThis.AUTO_GEAR_HAND_UNIT_MOTOR_TO_GROUP) === 'object' ? globalThis.AUTO_GEAR_HAND_UNIT_MOTOR_TO_GROUP : Object.create(null);
    var contextValue = '';
    ...
    Object.values(groups).forEach(function (entry) {
      ...
      entry.options.forEach(function (option) {
        if (!option || fallback_1.includes(option)) return;
...
function populateAutoGearCategorySelect(select, currentValue) {
  ...
  customOption.textContent = ((_a = texts === null || texts === void 0 ? void 0 : texts[lang]) === null || _a === void 0 ? void 0 : _a.autoGearCustomCategory) || ((_b = texts === null || texts === void 0 ? void 0 : texts.en) === null || _b === void 0 ? void 0 : _b.autoGearCustomCategory) || 'Custom Additions';
...
function refreshAutoGearOwnGearConditionOptions(selected) {
  ...
  var seen = new Set();
  var appendOption = function appendOption(id, label, options) {
    if (options === void 0) {
      options = {};
    }
...
function updateAutoGearOwnGearOptions() {
  ...
  selects.forEach(function (select) {
    ...
    var placeholder = select.getAttribute('data-placeholder') || ((_a = texts[currentLang]) === null || _a === void 0 ? void 0 : _a.autoGearOwnGearPlaceholder) || ((_b = texts.en) === null || _b === void 0 ? void 0 : _b.autoGearOwnGearPlaceholder) || 'Manual entry';
    select.innerHTML = '';
...
function normalizeAutoGearItem(entry) {
  ...
  var rawName = (ownGearRecord === null || ownGearRecord === void 0 ? void 0 : ownGearRecord.name) || ownGearNameInput || requestedName;
  if (!rawName) return null;
  var _a = extractAutoGearContextNotes(requestedName || rawName),
...
function normalizeAutoGearRule(rule) {
  ...
  if (scenarioLogic === 'multiplier') {
    ...
    if (normalizedPrimary_1) {
      var matched = scenarios.find(function (value) {
        return normalizeAutoGearTriggerValue(value) === normalizedPrimary_1;
      });
      if (matched) {
  ...
  if (scenarioLogic === 'multiplier' && scenarioPrimary) {
    ...
    var hasPrimary = scenarios.some(function (value) {
      return normalizeAutoGearTriggerValue(value) === normalizedPrimary_2;
    });
    if (!hasPrimary) {
...
if (typeof globalThis !== 'undefined') {
  var target = _typeof(globalThis.AUTO_GEAR_NORMALIZER_EXPORTS) === 'object' && globalThis.AUTO_GEAR_NORMALIZER_EXPORTS ? globalThis.AUTO_GEAR_NORMALIZER_EXPORTS : globalThis.AUTO_GEAR_NORMALIZER_EXPORTS = {};
