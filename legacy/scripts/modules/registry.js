...
(function () {
  ...
  function register(name, moduleApi) {
    ...
    if (Object.prototype.hasOwnProperty.call(moduleMap, normalizedName)) {
      ...
      if (!options.replace) {
        throw new Error("cineModules: Module \"".concat(normalizedName, "\" is already registered. Use { replace: true } to overwrite."));
      }
    }
    ...
    for (var index = 0; index < pending.length; index += 1) {
      ...
      try {
        ...
        if (typeof console !== 'undefined' && typeof console.warn === 'function') {
          console.warn("cineModules: discard pending registration for \"".concat(name, "\" due to error:"), error);
        }
        continue;
      }
    }
