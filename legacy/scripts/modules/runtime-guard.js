...
// ---------------------------------------------------------------------------
// Runtime guard module (legacy bundle)
// ---------------------------------------------------------------------------
// Mirrors the modern runtime guard helpers in an ES5-compatible format so the
// legacy bundle benefits from the same registry backfill and integrity checks.
// Keeping the logic centralised prevents data-loss regressions when the
// application is loaded in older browsers.
...
(function () {
  ...
  var safeWarn = MODULE_BASE && typeof MODULE_BASE.safeWarn === 'function'
  ...
  function attemptRegistryBackfill(scope) {
    ...
    for (var index = 0; index < registryCandidates.length; index += 1) {
      ...
      if (seen) {
        ...
        seen.add(candidate);
      } else if (registries.indexOf(candidate) !== -1) {
        continue;
      }
  ...
  function verifyRuntimeIntegrity(scope, options) {
    ...
    var warnOnFailure = settings.warnOnFailure !== false;
    var shouldThrow = settings.throwOnFailure !== false;

    var result;

    try {
      result = runtime.verifyCriticalFlows({ warnOnFailure: warnOnFailure });
    } catch (error) {
      var failure = freezeShallow({ ok: false, error: error });
    ...
    var normalizedResult = result && typeof result === 'object'
      ? result
  ...
  if (MODULE_BASE && typeof MODULE_BASE.registerOrQueueModule === 'function') {
    try {
      MODULE_BASE.registerOrQueueModule(
        ...
        function (error) {
          safeWarn('Unable to register cineRuntimeGuard module.', error);
        },
        GLOBAL_SCOPE,
