...
if (CORE_PART2_RUNTIME_SCOPE && CORE_PART2_RUNTIME_SCOPE.__cineCorePart2Initialized) {
  ...
  function corePart2Runtime() {
    ...
    function createDefaultSearchNormalizer() {
      ...
      return function (value) {
        ...
        for (var index = 0; index < ligatureEntries.length; index += 1) {
          var entry = ligatureEntries[index];
          var source = entry[0];
    ...
    var defaultFeatureSearchNormalizer = createDefaultSearchNormalizer();

    function createFeatureSearchFallback() {
      return {
        normalizeSearchValue: function normalizeSearchValue(value) {
          return defaultFeatureSearchNormalizer(value);
        },
        sanitizeHighlightTokens: function sanitizeHighlightTokens(tokens) {
          ...
          for (var index = 0; index < tokens.length; index += 1) {
            ...
            var normalized = defaultFeatureSearchNormalizer(token);
            if (!normalized) {
              continue;
            }
    ...
    var fallbackNormalizeSearchValue = defaultFeatureSearchNormalizer;
