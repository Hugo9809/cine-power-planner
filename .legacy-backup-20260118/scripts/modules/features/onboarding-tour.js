...
(function () {
  ...
  function getRootFontSizePx() {
    ...
    try {
      var view = docEl.ownerDocument && docEl.ownerDocument.defaultView ? docEl.ownerDocument.defaultView : DOCUMENT.defaultView || GLOBAL_SCOPE;
      if (view && typeof view.getComputedStyle === 'function') {
        var styles = view.getComputedStyle(docEl);
        if (styles && typeof styles.fontSize === 'string') {
          var parsed = parseFloat(styles.fontSize);
    ...
    var inlineValue = docEl && docEl.style && typeof docEl.style.fontSize === 'string' ? parseFloat(docEl.style.fontSize) : NaN;
  ...
  var visualViewportListenersAttached = false;
  var active = false;
  var currentIndex = -1;
  ...
  function isOverlayScrollEvent(event) {
    ...
    if (typeof event.composedPath === 'function') {
      try {
        ...
        if (Array.isArray(composedPath)) {
          for (var index = 0; index < composedPath.length; index += 1) {
        ...
        safeWarn('cine.features.onboardingTour could not inspect scroll event composedPath.', composedPathError);
      }
    }
    var target = event.target;
    if (target === overlayRoot) {
    ...
    if (overlayRoot && typeof overlayRoot.contains === 'function' && target && _typeof(target) === 'object') {
      try {
        ...
        safeWarn('cine.features.onboardingTour encountered an error inspecting scroll event containment.', containsError);
      }
    }
    return false;
  }
  ...
  function getOverlayMetrics() {
    ...
    var offsetLeft = 0;
    var offsetTop = 0;
    var viewportWidth = fallbackWidth;
    ...
    if (visualViewport) {
      var isAnchored = Boolean(overlayRoot && overlayRoot.classList && typeof overlayRoot.classList.contains === 'function' && overlayRoot.classList.contains('onboarding-overlay--anchored'));
      var visualWidth = typeof visualViewport.width === 'number' && visualViewport.width > 0 ? visualViewport.width : null;
      var visualHeight = typeof visualViewport.height === 'number' && visualViewport.height > 0 ? visualViewport.height : null;
      var rawOffsetLeft = typeof visualViewport.offsetLeft === 'number' ? visualViewport.offsetLeft : typeof visualViewport.pageLeft === 'number' ? visualViewport.pageLeft : 0;
      var rawOffsetTop = typeof visualViewport.offsetTop === 'number' ? visualViewport.offsetTop : typeof visualViewport.pageTop === 'number' ? visualViewport.pageTop : 0;
      var computedOffsetLeft = Number.isFinite(rawOffsetLeft) ? rawOffsetLeft : 0;
      var computedOffsetTop = Number.isFinite(rawOffsetTop) ? rawOffsetTop : 0;
      if (!isAnchored) {
  ...
  function updateHeroInlineSize(size, metrics, forcedRootFontSize) {
    ...
    var overlayMetrics = metrics || getOverlayMetrics();
    if (!overlayMetrics || typeof overlayMetrics.viewportWidth !== 'number') {
      cardEl.style.removeProperty('--onboarding-card-hero-inline-size');
      return;
    }
    var viewportWidth = overlayMetrics.viewportWidth;
    if (!Number.isFinite(viewportWidth) || viewportWidth <= 0) {
      cardEl.style.removeProperty('--onboarding-card-hero-inline-size');
      return;
    }
  ...
  function attachVisualViewportListeners() {
    ...
    var viewport = GLOBAL_SCOPE && GLOBAL_SCOPE.visualViewport;
  ...
  function positionCard(target, targetRect) {
    ...
    var viewportHeight = overlayMetrics && typeof overlayMetrics.viewportHeight === 'number' ? overlayMetrics.viewportHeight : 0;
