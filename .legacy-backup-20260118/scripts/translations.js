...
(function initTranslationsLoader() {
  ...
  var LOCALE_SCRIPTS = {
    en: {
      url: 'src/scripts/translations/en.js',
      preloaded: true
    },
    it: {
      url: 'src/scripts/translations/it.js'
    },
    es: {
      url: 'src/scripts/translations/es.js'
    },
    fr: {
      url: 'src/scripts/translations/fr.js'
    },
    de: {
      url: 'src/scripts/translations/de.js'
    }
  };
  ...
  function alignTranslationValue(referenceValue, translationValue) {
    ...
    if (referenceType === 'object') {
      ...
      var alignedObject = translationValue && _typeof(translationValue) === 'object' ? translationValue : {};
      ...
      for (var keyIndex = 0; keyIndex < referenceKeys.length; keyIndex += 1) {
        ...
        var translationChild = Object.prototype.hasOwnProperty.call(alignedObject, key) ? alignedObject[key] : undefined;
  ...
  function alignDataset(referenceDataset, localeDataset) {
    ...
    for (var index = 0; index < referenceKeys.length; index += 1) {
      ...
      var localeValue = localeDataset && Object.prototype.hasOwnProperty.call(localeDataset, key) ? localeDataset[key] : undefined;
  ...
  function registerLocaleData(locale, data) {
    ...
    var localeTexts = data.texts && _typeof(data.texts) === 'object' ? data.texts : {};
    var localeCategoryNames = data.categoryNames && _typeof(data.categoryNames) === 'object' ? data.categoryNames : {};
    var localeGearItems = data.gearItems && _typeof(data.gearItems) === 'object' ? data.gearItems : {};
      ...
      if (typeof require === 'function') {
        try {
          ...
          if (typeof console !== 'undefined' && typeof console.error === 'function') {
            console.error('Failed to require translation bundle for locale during offline load:', locale, requireError);
          }
          throw requireError;
        }
      }
    ...
    if (typeof require === 'function') {
      try {
        ...
        if (typeof console !== 'undefined' && typeof console.warn === 'function') {
          console.warn('Default locale require failed; deferring to asynchronous translation loader.', requireError);
        }
      }
    }
  ...
  function ensureGlobalLexicalBinding(name, value) {
    ...
    try {
      if (typeof scope.Function === 'function') {
        scope.Function('value', "try { if (typeof " + name + " === 'undefined') { " + name + " = value; } } catch (error) { void error; } return " + name + ';')(value);
      }
    } catch (lexicalError) {
      void lexicalError;
    }
